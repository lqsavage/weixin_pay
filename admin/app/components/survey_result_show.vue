<template>
   <div class="survey-result-show">
       <div class="row">
           <a class="btn btn-default pull-left back-btn" href="#/survey_result/list">返回</a>
       </div>
                       ...
                       <div v-for="(question, index) in survey.questions" :key="index">
                           ...
                           <div v-if="question.types === 'check'">
                               <div  class="checkbox" v-for="(option, i) in question.options" :key="i">
                                   <label>
                                       <input type="checkbox" :checked="questionResults[index].choose_option_ids?questionResults[index].choose_option_ids.indexOf(option.id)!=-1:false">{{ option.text }}
                                   </label>
                               </div>
                           </div>
...
<style>
  ...
  .survey-result-show .margin-T20{
     margin-top: 20px;
  }
  .survey-result-show .left-area{
      border-right: 1px solid #eee;
  }
  .survey-result-show .right-area{
      padding-left: 30px;
  }
  .survey-result-show .back-btn{
      margin-left: 15px;
      margin-bottom: 10px;
  }
</style>